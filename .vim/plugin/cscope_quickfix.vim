...
" 
" Taken from https://github.com/vim-scripts/cscope-quickfix
"
" Overview
" ------------
...
function! s:RunCscope(...)
	...
	if cscope_opt == '0' || cscope_opt == 's'
		let cmd = "cscope -LRq -0 " . pattern
	elseif cscope_opt == '1' || cscope_opt == 'g'
		let cmd = "cscope -LRq -1 " . pattern
	elseif cscope_opt == '2' || cscope_opt == 'd'
		let cmd = "cscope -LRq -2 " . pattern
	elseif cscope_opt == '3' || cscope_opt == 'c'
		let cmd = "cscope -LRq -3 " . pattern
	elseif cscope_opt == '4' || cscope_opt == 't'
		let cmd = "cscope -LRq -4 " . pattern
	elseif cscope_opt == '6' || cscope_opt == 'e'
		let cmd = "cscope -LRq -6 " . pattern
	elseif cscope_opt == '7' || cscope_opt == 'f'
		let cmd = "cscope -LRq -7 " . pattern
		let openwin = 0
		let jumperr = 1
	elseif cscope_opt == '8' || cscope_opt == 'i'
		let cmd = "cscope -LRq -8 " . pattern
	else
		echohl WarningMsg | echomsg usage | echohl None
		return
	endif
  ...
  " Hack it in so that if we jump inside of the CScope window, we don't
  " necessarily go into the reference
  nnoremap <buffer> <CR> <CR><C-W>p

  " Make this window have a special var so we can exit at the end without
  " closing all the windows
  let b:IsCscopeBuffer=1
